...
export default function LearningStatsHub({
  ...
  const cardBase = `group relative min-w-[132px] flex-1 basis-[calc(50%-0.25rem)] cursor-pointer overflow-hidden rounded-2xl border p-2.5 text-left transition-all duration-300 sm:min-w-[160px] lg:min-w-0 lg:basis-auto lg:p-3 ${
    isLight
  ...
  const getStreakBarClass = (active: boolean) => {
    if (!active) return isLight ? "bg-gray-100" : "bg-white/10";
  ...
  return (
    <>
      <div className="w-full">
        <div className="flex flex-wrap gap-2 lg:grid lg:grid-cols-3 lg:gap-3">
          <button
            type="button"
          ...
          >
            <div className="absolute -right-4 -top-5 opacity-10">
              <StreakIcon className="h-14 w-14 lg:h-20 lg:w-20" />
            </div>

            <div className="flex items-start justify-between gap-2">
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-xl lg:h-9 lg:w-9 ${getStreakIconClass()}`}
              >
                <motion.div
                  animate={
                ...
                >
                  <StreakIcon className="h-3.5 w-3.5 lg:h-[18px] lg:w-[18px]" />
                </motion.div>
              </div>

              <span
                className={`rounded-full px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wide lg:px-2 lg:py-1 lg:text-[9px] ${getStreakPillClass()}`}
              >
                {streakCopy.label}
              </span>
            </div>
            ...
            <p
              className={`mt-2 text-[9px] font-bold uppercase tracking-wide lg:mt-3 lg:text-[11px] ${
            ...
            <div className="mt-1 flex items-end gap-1">
              <span
                className={`text-xl font-black leading-none lg:text-3xl ${strongText}`}
              >
                {streak}
              </span>
              <span className={`text-[10px] lg:text-xs ${mutedText}`}>
                days
              </span>
            </div>

            <div className="mt-2 flex gap-1 lg:mt-3">
              {Array.from({ length: 7 }).map((_, index) => (
                <span
                  key={index}
                  className={`h-1 flex-1 rounded-full lg:h-1.5 ${getStreakBarClass(
            ...
            <p
              className={`mt-2 text-[9px] leading-4 lg:mt-3 lg:text-[11px] lg:leading-5 ${mutedText}`}
            >
              {streakCopy.short}
            </p>
          </button>
          ...
          >
            <div className="absolute -right-4 -top-5 opacity-10">
              <Target className="h-14 w-14 lg:h-20 lg:w-20" />
            </div>

            <div className="flex items-start justify-between gap-2">
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-xl lg:h-9 lg:w-9 ${
                  isLight
                    ? "bg-emerald-100 text-emerald-500"
                    : "bg-emerald-500/15 text-emerald-300"
                }`}
              >
                <Target className="h-3.5 w-3.5 lg:h-[18px] lg:w-[18px]" />
              </div>

              <span
                className={`rounded-full px-1.5 py-0.5 text-[7px] font-black uppercase tracking-wide lg:px-2 lg:py-1 lg:text-[9px] ${
                  isLight
                    ? "bg-emerald-50 text-emerald-700"
                    : "bg-emerald-500/10 text-emerald-300"
                }`}
              >
                L{Math.max(1, Math.floor(completedLessons / 10) + 1)}
              </span>
            </div>
            ...
            <div
              className={`mt-2 h-1.5 overflow-hidden rounded-full lg:mt-3 lg:h-2 ${
          ...
          >
            <div className="absolute -right-4 -top-5 opacity-10">
              <Trophy className="h-14 w-14 lg:h-20 lg:w-20" />
            </div>
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              >
                <Trophy className="h-3.5 w-3.5 lg:h-[18px] lg:w-[18px]" />
              </div>
              ...
              {earnedMilestones.length > 0
                ? earnedMilestones.slice(0, 4).map((badge, index) => (
                    <motion.div
                      ...
                      className={`flex h-6 w-6 items-center justify-center rounded-lg lg:h-7 lg:w-7 ${
                        isLight
            ...
            >
              {earnedMilestones.length > 0
                ? "Unlocked."
                : `Next: ${lessonsToNextBadge} lesson${
            ...
            >
              ...
              <div className="mt-4 space-y-3">
                ...
                {activePanel === "progress" && (
                  <>
                    ...
                    >
                      <p className={`text-xs leading-5 ${mutedText}`}>
                        You have completed <strong>{completedLessons}</strong>{" "}
                        out of <strong>{totalLessons}</strong> lessons and{" "}
              ...
              {onContinueLearning && (
                <button
                  ...
                  className="mt-4 flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-pink-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-pink-700"
                >
                  Continue Learning
